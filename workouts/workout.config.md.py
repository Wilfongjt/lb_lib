# ported to source/lb_config_md.py
import os
import re
...
## 
'''
md_expected={'project': {}, 'what': []}

class ConfigMd(LbRecorder):
    def __init__(self):
        super().__init__()
        self.text=''
...
def main():
    ...
    assert (actual.getValue('# a: 1') == 1)
    assert (actual.getValue('# a:1.1') == 1.1)
    assert (actual.getValue('# a: 1.1') == 1.1)
    ...
    assert (actual.getValue('1. a:, b:B, c:True, d:False, e:1, f:1.1') == {'a': '', 'b': 'B', 'c': True, 'd': False, 'e': 1, 'f': 1.1})
    ...
    ### model: []
    1. name: id, type:C, size:3-330, validate:R 
    1. name: type, type:C, size:3-330, validate:R
    1. name: owner, type:C, size:3-330, validate:R
    1. name: username, type:C, size:3-30, validate:R
    1. name: password, type:C, size:8-330, validate:R
    ...
    md5expected={'model': []}
    actual = ConfigMd()
    #print('actual', actual.set_md_text(md1).toJSON())
    #print('actual', actual.getSteps())
    assert (actual.set_md_text(md1).toJSON(show=False)=={'project': {'name': 'TBD', 'claims': {'name': 'TBD'}, 'roles': ['api_admin', 'api_guest', 'api_user'], 'resources': {'account': {}, 'picture': {}}}})
    assert (actual.set_md_text(md2).toJSON(show=True)=={'project': {'name': 'example', 'version': 1.0, 'claims': {'issuer': 'lyttlebit', 'audience': 'example-api_client', 'subject': 'client_api'}, 'roles': ['api_admin', 'api_guest', 'api_user'], 'account': {'scope': {'api_guest': ['C'], 'api_user': ['R', 'U', 'D']}, 'model': [{'name': 'id', 'type': 'C', 'size': {'min': 3, 'max': 330}, 'validate': 'R'}, {'name': 'type', 'type': 'C', 'size': {'min': 3, 'max': 330}, 'validate': 'R'}, {'name': 'owner', 'type': 'C', 'size': {'min': 3, 'max': 330}, 'validate': 'R'}, {'name': 'username', 'type': 'C', 'size': {'min': 3, 'max': 30}, 'validate': 'R'}, {'name': 'password', 'type': 'C', 'size': {'min': 8, 'max': 330}, 'validate': 'R'}]}}})
    assert (actual.set_md_text(md3).toJSON(show=False)=={'model': [{'name': 'id', 'type': 'C', 'size': {'min': 3, 'max': 330}, 'validate': 'R'}, {'name': 'type', 'type': 'C', 'size': {'min': 3, 'max': 330}, 'validate': 'R'}, {'name': 'owner', 'type': 'C', 'size': {'min': 3, 'max': 330}, 'validate': 'R'}, {'name': 'username', 'type': 'C', 'size': {'min': 3, 'max': 30}, 'validate': 'R'}, {'name': 'password', 'type': 'C', 'size': {'min': 8, 'max': 330}, 'validate': 'R'}]})
    assert (actual.set_md_text(md4).toJSON()=={'model': [{'name': 'id', 'type': 'C', 'size': {'min': 3, 'max': 330}, 'validate': 'R'}]})
    assert (actual.set_md_text(md5).toJSON()=={'model': []})
